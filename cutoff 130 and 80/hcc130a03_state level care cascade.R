...
state_svysummary %>% 
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>% 
            ...
            by=c("state"="v024")) %>% 
write_csv(.,file = "cutoff 130 and 80/hcc130a03_state level care cascade.csv")


# df <- read_csv(file="cutoff 130 and 80/hcc130a03_state level care cascade.csv") %>%
#   dplyr::select(-n5_state,-contains("zone")) %>%
#   left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>%
#               dplyr::select(v024,n5_state,zone) %>%
#               distinct(v024,n5_state,.keep_all=TRUE),
#             by=c("state"="v024"))
# write_csv(df,file = "cutoff 130 and 80/hcc130a03_state level care cascade.csv")
