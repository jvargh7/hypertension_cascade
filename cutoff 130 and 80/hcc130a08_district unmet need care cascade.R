...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),"mapnfhs5_sdist") %>% 
            ...
            by=c("REGCODE"))  %>% 
  write_csv(.,file = "cutoff 130 and 80/hcc130a08_district unmet need care cascade.csv")
...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),"mapnfhs5_sdist") %>% 
              dplyr::select(REGCODE,n5_state,v024,REGNAME),
            by=c("REGCODE"))  %>% 
  write_csv(.,file = "cutoff 130 and 80/hcc130a08_district met need care cascade.csv")

# df <- read_csv("cutoff 130 and 80/hcc130a08_district unmet need care cascade.csv") %>%
...
#             by=c("D_CODE"))
# write_csv(df,file = "cutoff 130 and 80/hcc130a08_district unmet need care cascade.csv")
# 
