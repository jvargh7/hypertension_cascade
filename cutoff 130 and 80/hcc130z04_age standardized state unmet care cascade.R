...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>% 
            ...
            by=c("state"="v024")) %>% 
  write_csv(.,file = "cutoff 130 and 80/hcc130z04_state unmet need care cascade.csv")
...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>% 
              ...
              distinct(v024,n5_state,.keep_all=TRUE),
            by=c("state"="v024")) %>% 
  write_csv(.,file = "cutoff 130 and 80/hcc130z04_state met need care cascade.csv")
