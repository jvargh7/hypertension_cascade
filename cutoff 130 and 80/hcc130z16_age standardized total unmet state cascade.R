...
nfhs5htnz_svydesign = svystandardize(nfhs5htn_svydesign,by=~age_category,over = ~state + residence,
                                     population = pop_age)
nfhs5htndiagz_svydesign = svystandardize(nfhs5htndiag_svydesign,by=~age_category,over = ~state + residence,
                                         population = pop_age)

nfhs5htntreatz_svydesign = svystandardize(nfhs5htntreat_svydesign,by=~age_category,over = ~state + residence,
                                          population = pop_age)
...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>% 
            ...
            by=c("state"="v024")) %>% 
  write_csv(.,file = "cutoff 130 and 80/hcc130z16_total state unmet need care cascade.csv")
...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>% 
              ...
              distinct(v024,n5_state,.keep_all=TRUE),
            by=c("state"="v024")) %>% 
  write_csv(.,file = "cutoff 130 and 80/hcc130z16_total state met need care cascade.csv")
