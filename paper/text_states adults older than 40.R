# Based on hca05_state unmet need care cascade -------

group_vars = c("age_category2")
...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>% 
            ...
            by=c("state"="v024")) %>% 
  write_csv(.,file = "paper/text_adults older than 40 state unmet need care cascade.csv")
...
bind_rows(unmet_svysummary_htn,
  ...
  left_join(readxl::read_excel(paste0(path_dmcascade_repo,"/data/NFHS Cascade Variable List.xlsx"),sheet="map2020_v024") %>% 
              ...
              distinct(v024,n5_state,.keep_all=TRUE),
            by=c("state"="v024")) %>% 
  write_csv(.,file = "paper/text_adults older than 40 state met need care cascade.csv")
