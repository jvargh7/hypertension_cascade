...
lasi_sy <- svysummary(lasipop_design,
...
lasi_out %>% 
  write_csv(.,file = "qc/lasiqc02_equivalent population characteristics.csv")


lasi_out %>% 
  dplyr::select(strata,variable,group,est_ci) %>% 
  pivot_wider(names_from=c("strata"),values_from="est_ci") %>% 
  write_csv(.,"qc/lasiqc_total population characteristics.csv")
