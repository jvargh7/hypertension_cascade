...
lasi_sy <- svysummary(lasipopipw_design,
...
lasi_out %>% 
  write_csv(.,file = "qc/lasiqc03_ipw population characteristics.csv")


lasi_out %>% 
  dplyr::select(strata,variable,group,est_ci) %>% 
  pivot_wider(names_from=c("strata"),values_from="est_ci") %>% 
  write_csv(.,"qc/lasiqc_ipw population characteristics.csv")
