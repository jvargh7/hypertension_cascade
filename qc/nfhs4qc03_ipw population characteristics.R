...
nfhs4_sy <- svysummary(nfhs4popipw_design,
...
nfhs4_out %>% 
  write_csv(.,file = "qc/nfhs4qc03_ipw population characteristics.csv")


nfhs4_out %>% 
  # read_csv("qc/nfhs4qc03_ipw population characteristics.csv") %>% 
  dplyr::select(strata,variable,group,est_ci) %>% 
  pivot_wider(names_from=c("strata"),values_from="est_ci") %>% 
  write_csv(.,"qc/nfhs4qc_ipw population characteristics.csv")
